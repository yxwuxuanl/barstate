...
BarState 是一款 macOS 菜单栏监控工具。它会按设定周期查询服务余额、HTTP API、Prometheus 或 Codex 额度，从响应中取得一个数值，再将结果显示在菜单栏中。

本文描述 BarState 1.0.0。已通过及本轮跳过的验收见 [实施记录](ITERATION_PROGRESS.md)，版本变化见 [版本记录](../CHANGELOG.md)。[English guide](USER_GUIDE_EN.md)。
...
2. 点击左侧“新增”，在来源列表中选择“HTTP API”。
3. 名称填写 `室外温度`。
4. 在“菜单栏显示”中将显示模板填写为 `温度 ${value}℃`。
5. 返回“连接与解析”，填写接口的 HTTPS URL。
...
设置窗口左侧是监控项列表，显示名称、来源与当前值或运行状态。右侧顶部保留名称、启用开关和状态摘要，底部始终保留保存与撤销操作。

右侧三个分区共用同一份草稿：

- **连接与解析**：连接信息、指标或查询、刷新周期，以及测试结果。完整响应默认折叠。
- **菜单栏显示**：显示开关、必须包含 `${value}` 的模板、实际值预览和颜色规则。
- **提醒**：默认关闭的异常规则、阈值、恢复通知和系统权限说明。

切换分区不会丢失草稿；切换监控、进入“通用设置”或关闭窗口时，未保存修改会受到保护。保存错误会带你回到对应分区。左下方“通用设置”集中管理语言、菜单栏模式、标题长度和登录启动。右键或左下方操作菜单可克隆、移动、删除监控项。

对于已经保存的监控项，“启用监控”和“显示在菜单栏”两个开关会立即生效；其他修改需要点击“保存”。

### 添加服务预设

点击“新增”，在“AI 额度与余额”中选择服务。每个监控项选择一个指标；同一服务可创建多项。填写 Key 和必要选项后，先测试，再保存。预设默认每 5 分钟刷新，可自行调整。

| 来源 | 可选指标 | 配置与边界 |
| --- | --- | --- |
| DeepSeek | 账户总余额 | 选择 CNY 或 USD，按接口返回币种匹配；真实负余额不会被改成 0。 |
| OpenRouter | 今日用量、本月用量、Key 剩余预算 | 使用普通 API Key，单位 USD；未设置 Key 预算时，切换到用量指标，或先在服务端设置预算。 |
| SiliconFlow 中国站 | 总余额 | 使用中国站 Key 和 CNY；当前不提供国际站预设。 |

预设自动处理固定接口、解析和单位。凭据或指标改变后需重新测试。字段缺失、无预算、业务失败不会当作 0；已有监控会保留最后成功值并说明更新失败。余额接口的更新时间仍取决于服务方。
...
<p align="center">
  <img src="images/iteration-prometheus-en-dark.jpg" alt="Prometheus 模板设置，英文深色最小窗口，示例数据" width="760">
</p>

1. 点击“新增”，选择 `Prometheus`。
2. 填写 Prometheus 地址，例如 `https://prometheus.example.com`。
3. 选择 CPU、内存、磁盘或目标采集状态模板，填写目标的 `job` 和 `instance`；磁盘还需挂载点。也可以选择自定义查询并填写 PromQL。
4. 如有需要，配置 Basic Authentication 或添加认证 Header。
5. 点击“测试查询”。查询成功时会直接显示取得的数值。
6. 设置显示模板和刷新周期，保存并启用监控。

资源模板使用 Linux node_exporter 指标，前三项输出百分比。目标采集状态使用 `up`：`1` 表示抓取成功，`0` 表示抓取失败，不等于整台主机离线。有效的 `0` 可用于数值提醒；BarState 无法查询 Prometheus 则属于请求失败。默认每分钟刷新。
...
| 正在刷新 | 请求正在进行。 |
| 运行正常 | 最近一次请求和解析成功。 |
| 数据已过期 | 距最后成功取数超过 `2 × 刷新周期 + 请求超时`，或检测到明显的系统时间回拨；旧值不会被当作新数据。 |
| 本机网络离线 | 远端监控等待网络恢复；本机环回 Prometheus 仍可查询。 |
...
菜单栏、弹窗和设置共用状态判断。展开状态可查看最后成功时间和错误原因；异常与过期标记独立于数值颜色。唤醒或网络恢复后通过新请求确认恢复，不直接清除旧错误。

### 开启异常提醒

在监控的“提醒”分区开启规则，选择“小于等于阈值”“大于等于阈值”或“连续请求失败”，按需允许系统通知，保存后生效。每项只有一条规则，默认关闭；克隆出来的监控也默认关闭提醒。

| 条件 | 异常确认 | 恢复确认 |
| --- | --- | --- |
| 数值阈值 | 连续两次成功取数满足条件 | 连续两次成功取数不满足条件 |
| 请求失败 | 连续三次失败 | 一次成功取得有效数值 |

例如阈值为 `≤ 20`，数值 `21 → 19 → 18 → 17` 只产生一次异常通知；随后 `21 → 22` 确认恢复。恢复通知需单独开启。再次异常距上次异常通知至少 15 分钟，并由下一次检查确认；重启会保留必要的去重状态。

测试连接、旧缓存、缺失字段不会触发数值提醒。本机离线时不为每个远端监控分别发送故障通知。拒绝系统权限不会影响监控；可以稍后在系统设置中允许通知。点击通知会打开设置并选中对应监控；已删除的监控不会再被打开。
...
BarState 不提供自己的云端账户。打包应用的配置和最近运行状态保存在：

```text
~/Library/Containers/com.barstate.BarState/Data/Library/Application Support/BarState/
```

未使用应用沙盒的源码运行环境可能使用 `~/Library/Application Support/BarState/`。主要文件为 `state.json` 和 `state.backup.json`，文件权限会限制为仅当前 macOS 用户可读写。

如果主文件损坏但备份可读取，BarState 会以只读方式载入备份，等待你恢复或导出副本。如果两个文件都无法读取，应用会阻止所有配置写入；选择“归档并重新开始”后，旧文件会先重命名保留，再创建空配置。

Basic Authentication 用户名与密码、预设 API Key、请求头的完整值保存在这些本地文件中，尚未使用钥匙串。自定义 HTTP/Prometheus 保存最近响应；服务预设仅保留必要指标与诊断摘要，Codex 仅保留限额对象且不复制凭据。提醒规则与去重状态也保存在本机；通知不包含 URL、Key、请求头或完整响应。建议：
...
当前查询返回了多个元素。检查 `job`、`instance`、磁盘挂载点及重复采集配置，增加标签筛选。只有确实需要汇总同一目标时才使用 `sum()`、`avg()`，不要用聚合掩盖目标选择错误。

...
BarState 是一款 macOS 菜单栏监控工具。它会按设定周期查询 HTTP API、Prometheus 或 Codex 额度，从响应中取得一个数值，再将结果显示在菜单栏中。适合查看温度、汇率、API 额度、服务负载、错误率等需要随时关注的指标。
...
设置窗口左侧是监控项列表，右侧是当前监控项的配置和运行状态。首次使用时可以直接选择 HTTP API、Prometheus、Codex Quota 或 JSON 数值模板。新建流程会按照“连接 → 提取/查询/额度 → 展示”显示当前进度，认证、请求头和超时位于可折叠的高级设置中。
...
Codex Quota 模式显示本机当前登录 Codex 账号的主要限额周期剩余百分比。

1. 先登录 Codex，确保 `~/.codex/auth.json` 已存在。
2. 在 BarState 左侧“新增”菜单中选择“新建 Codex Quota 监控”。
3. 点击“测试额度”，确认成功取得剩余百分比。
4. 设置显示模板和刷新周期，保存并启用监控。

BarState 固定调用 `https://chatgpt.com/backend-api/wham/usage`，并按 `100 - used_percent` 计算显示值。每次请求时都会从 `~/.codex/auth.json` 读取 `access_token` 和 `account_id`，但不会将凭据复制到 BarState 配置。应用对该文件只有只读访问权限。

为避免保留账号身份信息，Codex 响应写入本地状态前只保留 `rate_limit` 对象，不保存用户 ID、账号 ID 或邮箱地址。新建 Codex Quota 监控项前必须成功完成“测试额度”。
...
BarState 不提供自己的云端账户。配置和最近运行状态保存在：
...
Basic Authentication 用户名与密码、请求头的完整值和最近一次完整响应会以明文保存在这些本地文件中。Codex 凭据不会复制到 BarState，Codex 响应只保留限额数据。即使界面遮挡了密码或敏感 Header，它也不等于使用钥匙串加密。建议：
...
### Codex Quota 提示找不到认证文件

先确认已经在本机登录 Codex，并且 `~/.codex/auth.json` 存在。如果文件存在但缺少 access token，请重新登录 Codex 后再点击“测试额度”。
